 ...
 * @version 1.2
 */
declare type GridQueryResult = Grid | undefined;
/** 棋盤格查詢暫存器 */
declare class GridQueryCache {
    /** 查詢結果暫存(單數) */
    protected _queryCache: {
        [command: string]: GridQueryResult;
    };
    /** 查詢結果暫存(複數) */
    ...
    /** 暫存查詢結果(單數) */
    protected _cacheQueryResult(command: string, result: GridQueryResult): GridQueryResult;
    /** 暫存查詢結果(複數) */
    protected _cacheQueriesResult(commmands: string, results: GridQueryResult[]): GridQueryResult[];
}
/** 棋盤格 */
export declare class Grid extends GridQueryCache {
     ...
     * query("-BR");
     */
    query(directionCommand: string): GridQueryResult;
    /**
     * 使用相對座標進行棋盤格查詢，查詢後暫存，
     * 若下次有相同查詢請求時將直接返回查詢結果
     * 相對座標格式擴充:
     ...
     * queries("3~F");
     */
    queries(directionCommands: string): GridQueryResult[];
    /**
     * 簡寫座標轉換為一般座標
     * @param directionCommands 相對座標指令(複數)
     *
     ...
     */
    static getOriginalDirection(directionCommands: string): string[];
}
/** 棋盤 */
export declare class GridBoard extends GridQueryCache {
     ...
     * query("E3");
     */
    query(coordinateCommand: string): GridQueryResult;
    /**
     * 使用絕對座標進行棋盤格查詢，查詢後暫存，
     * 若下次有相同查詢請求時將直接返回查詢結果
     * 絕對座標格式擴充:
     ...
     * queries("A5:B4");
     */
    queries(coordinateCommands: string): GridQueryResult[];
}
export {};
