...
const BY_NORMAL_RELATI = 0;
const BY_COMMON_RELATI = 1;

function getRelatiRoutesBy(grid: RelatiGrid, status: number, routeType: number) {
...
function hasRelatiRoutesBy(grid: RelatiGrid, status: number, routeType: number) {
    switch (routeType) {
        case BY_COMMON_RELATI:
            for (let i = 0; i < 24; i++) {
                let targetGrid = grid.getGrid(REMOTE_STABLE_RELATI_ROUTES[i][0]);
