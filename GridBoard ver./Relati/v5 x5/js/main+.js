        ...
        for (var crd in lib.board.gridOf) {
            ...
            if (grid.is("space-real")) {
                if (grid.by("relati", sym).length > 0) {
                    ...
                    lib.board.viewer.appendGridDot(grid, 0.2, lib.SymtusColor[SymtusSymbol[sym]]);
                }
            }
        }
    }
    ...
    lib.board.viewer.onselect = function (grid) {
        ...
        if (grid.is("space-real")) {
            if (lib.turn < lib.symbols.length) {
                ...
                for (var crd in lib.board.gridOf) {
                    var grid_4 = lib.board[crd];
                    if (grid_4.is("owner", sym) && grid_4.is("valid")) {
                        grid_4.to("broken");
                    }
                }
                grid.symbol = sym;
            ...
            var bullets = grid.by("attack", sym);
            if (bullets.length > 0) {
                bullets.forEach(function (grid) { return grid.to("select"); });
                lib.bulletSelect = true;
                grid.to("broken");
                return;
            }
        }
