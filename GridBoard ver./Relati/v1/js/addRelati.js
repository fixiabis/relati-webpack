(function () {
    ...
    var theme = {
        ...
        status: {
            "normal": function (grid) { return updateView(grid, {
                "stroke-width": "0.6",
            ...
            "forbid": function (grid) { return updateView(grid, { "stroke": "#888" }); }
        }
    };
    ["O", "X", "D", "U"].forEach(function (sym) {
        theme.status[sym + ".next"] = function (grid) { return createDot(grid, sym, "0.4"); };
    });
    ["O", "X", "D", "U"].forEach(function (sym) {
        theme.status[sym + ".terr"] = function (grid) { return createDot(grid, sym, "0.2"); };
    });
    /**
     * 建立圓點於SVG
     * @param grid 棋盤格
    ...
    function appendView(grid, removeAnimate) {
        for (var i = 0; i < grid.views.length; i++) {
            if (!removeAnimate)
                grid.views[i].style.animation = "blink 0.8s 2";
                    ...
                    if (sourceGrid.is("owner valid", sym)) {
                        ...
                        if (spaceGrids[0].is("space", sym) && spaceGrids[1].is("space", sym)) {
                            viewer.appendGridPath([
                                sourceGrid, spaceGrids[0], spaceGrids[1], this
                            ]);
                            relatiable = true;
                        }
                        if (spaceGrids[1].is("space", sym) && spaceGrids[3].is("space", sym)) {
                            viewer.appendGridPath([
                                sourceGrid, spaceGrids[3], spaceGrids[1], this
                            ]);
                            relatiable = true;
                        }
                        if (spaceGrids[2].is("space", sym) && spaceGrids[3].is("space", sym)) {
                            viewer.appendGridPath([
                                sourceGrid, spaceGrids[3], spaceGrids[2], this
                            ]);
                            relatiable = true;
                        }
                        if (relatiable) {
