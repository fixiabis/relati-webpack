class ViewableBoard extends GridBoard {
        ...
        for (var i = 0; i < w; i++)
            for (var j = 0; j < h; j++)
                (function (x, y) {
                    ...
                    this.grids[crd].removeAllStatus = function (status) {
                        this.removeAllStatusFromGrid(status, this.grids[crd]);
                    }.bind(this);
                }).bind(this)(i, j);
        this.view = view;
    ...
    refresh() {
        ...
        painter.setTransform(1, 0, 0, 1, 0.5, 0.5);
        this.gridSize = gridSize;
        var lineCoor = this._.gridMode == "dot" ? (gridSize + 1) / 2 : 0;
        for (var i = 0; i <= h; i++) {
            painter.beginPath();
    ...
    removeAllStatusFromGrid(status, grid) {
        for (var statusName in grid._)
